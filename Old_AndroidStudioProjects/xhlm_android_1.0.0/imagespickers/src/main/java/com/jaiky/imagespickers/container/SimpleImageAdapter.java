...
public class SimpleImageAdapter extends CommonAdapter<String> {
	...
	private void initImgSize() {
		//带删除
        int displayWidth = Utils.displayWidth(mContext);
		if (isDelete) {
            // 如果行数为4，中间间隔3个3dp共9dp + 距离右边4个8dp共32dp
            int size = displayWidth - Utils.dip2px(mContext, 3 * (rowCount -1) + 8 * rowCount);
		...
		//不带删除
			...
			int size = displayWidth - Utils.dip2px(mContext, 10 * (rowCount -1));
			size = Math.round(size / (float)rowCount);
