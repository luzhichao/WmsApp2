...
public class SuperEditText extends EditText {
    ...
    private Drawable icon_delete; // 删除图标
    private int icon_deleteResID; // 删除图标资源ID
    private int delete_x, delete_y, delete_width, delete_height; // 删除图标起点(x,y)、删除图标宽、高（px）

    private Drawable icon_leftClick, icon_leftUnClick; // 左侧图标（点击 & 未点击）
    private int icon_leftClickResID, icon_leftUnClickResID;    // 左侧图标资源ID（点击 & 无点击）
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // a. 点击状态的左侧图标
        // 1. 获取资源ID
        icon_leftClickResID = typedArray.getResourceId(R.styleable.SuperEditText_leftIconClick, R.drawable.user_select);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        icon_leftClick = getResources().getDrawable(icon_leftClickResID);
        // 3. 设置图标大小
        ...
        icon_leftClick.setBounds(left_x, left_y, left_width, left_height);
        // Drawable.setBounds(x,y,width,height) = 设置Drawable的初始位置、宽和高等信息
        // x = 组件在容器X轴上的起点、y = 组件在容器Y轴上的起点、width=组件的长度、height = 组件的高度

        // b. 未点击状态的左侧图标
        // 1. 获取资源ID
        icon_leftUnClickResID = typedArray.getResourceId(R.styleable.SuperEditText_leftIconUnClick, R.drawable.user_unselect);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        // 3. 设置图标大小（此处默认左侧图标点解 & 未点击状态的大小相同）
        icon_leftUnClick = getResources().getDrawable(icon_leftUnClickResID);
        icon_leftUnClick.setBounds(left_x, left_y, left_width, left_height);
        ...
        // 1. 获取资源ID
        icon_deleteResID = typedArray.getResourceId(R.styleable.SuperEditText_IconDelete, R.drawable.del_but_bg);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        icon_delete = getResources().getDrawable(icon_deleteResID);
        // 3. 设置图标大小
        // 起点(x，y)、宽= left_width、高 = left_height
        ...
        icon_delete.setBounds(delete_x, delete_y, delete_width, delete_height);

        /**
         * 设置EditText左侧 & 右侧的图片（初始状态未点击任何view，仅有左侧图片））
         */
        setCompoundDrawables(icon_leftUnClick, null,
                null, null);
...
//        }
    ...
    private void setDeleteIconVisible(boolean deleteVisible,boolean leftVisible) {
        setCompoundDrawables(leftVisible ?  icon_leftClick :  icon_leftUnClick, null,
                deleteVisible ?  icon_delete: null, null);
